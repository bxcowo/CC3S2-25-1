...
Puedes verificar al inicio de que las pruebas no pasan con pytest (Esto realmente resulta ser un erróneo, ya que al probar nuevamente el comando posterior nuestras pruebas son admitidas y no equívocas):

```
pytest -v
```

Se adjuntará una captura como referencia:
...
Aqui podremos ejecutar este comando, pero antes se necesita instalarlo en nuestro entorno virtual, ejecutamos:
```
...
Algo a tener en cuenta en los siguientes pasos es que las funciones que se desean definir ya vienen predefinidas en los archivos de `test_stack.py` durante las secciones que requieran de estas implementaciones se añadirán capturas de dichas funciones y sus funcionalidades.
...
En la implementación de la función `test_is_empy` contamos con la declaración de un objeto del tipo `Stack` y evaluamos con un `assertTrue` si es que este es considerado vacío mediante la función `is_empty()`, luego se introduce un nuevo valor 5 de modo que luego se evalúa con un `assertFalse` que este no está vacío.
...
En esta implementación de la función `test_peek` contamos con la nueva creación de un objeto del tipo `Stack` al cual introducimos dos valores 1 y 2; además, luego se evalúa mediante dos `assertEqual` si el valor devuelto por `peek()` es el último ingresado; es decir 2; por último se llama nuevamente a esta función para evaluar que el último no cambiase.
...
En la implementación de la función `test_pop` declaramos un objeto del tipo `Stack` y en el introducimos dos valores 1 y 2. Hacemos uso de `assertEqual` para verificar que la función `pop()` nos devuelve el último elemento ingresado, osea el 2; mientras que luego volvemos a hacer uso de `assertEqual` con la función `peek()` para verificar que ahora el último elemento sea el 1.
...
Para la implementación de la función `test_push` declaramos un objeto del tipo `Stack` con el que ingresamos un primer valor 1 y usamos `assertEqual` usando la función `peek()` para verificar que el último elemento en la pila es 1. Luego ingresamos un nuevo valor 2 a la pila y usamos la función `assertEqual` para validar este cambio en el último elemento de la pila.
...
pytest -v
```
Si todo está correcto, todas las pruebas deberían pasar y la salida mostrará resultados exitosos. Esto se comprobo al inicio que todas las pruebas eran completadas con normalidad.
...
```
Esto te mostrará cuántas líneas de código están cubiertas por las pruebas y cuáles no. Podemos ver el resultado del uso de dicho comando:
